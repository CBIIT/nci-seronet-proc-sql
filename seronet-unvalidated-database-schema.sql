...
CREATE TABLE `Comorbidity` (
  ...
  `Other_Comorbidity` varchar(255) COMMENT 'This should be a ICD-10 code.'
);
...
CREATE TABLE `Aliquot_Tube` (
  `Aliquot_Tube_Type_Lot_Number` varchar(255) PRIMARY KEY,
...
CREATE TABLE `Submission_MetaData` (
  `Submission_ID` varchar(255) COMMENT 'This is the file id assigned by SeroNet to the Zipped File',
...
ALTER TABLE `Prior_Test_Result` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Prior_Covid_Outcome` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Covid_Symptom` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Comorbidity` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Biospecimen` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Confirmatory_Test_Result` ADD FOREIGN KEY (`Research_Participant_ID`) REFERENCES `Demographic_Data` (`Research_Participant_ID`);

ALTER TABLE `Aliquot` ADD FOREIGN KEY (`Biospecimen_ID`) REFERENCES `Biospecimen` (`Biospecimen_ID`);

ALTER TABLE `Equipment` ADD FOREIGN KEY (`Biospecimen_ID`) REFERENCES `Biospecimen` (`Biospecimen_ID`);

ALTER TABLE `Reagent` ADD FOREIGN KEY (`Biospecimen_ID`) REFERENCES `Biospecimen` (`Biospecimen_ID`);

ALTER TABLE `Biorepository_Aliquot` ADD FOREIGN KEY (`Biorepository_ID`) REFERENCES `Aliquot` (`Biorepository_ID`);

ALTER TABLE `Assay_Target` ADD FOREIGN KEY (`Assay_ID`) REFERENCES `Confirmatory_Test_Result` (`Assay_ID`);

ALTER TABLE `Assay_Target` ADD FOREIGN KEY (`Assay_ID`) REFERENCES `Assay_Metadata` (`Assay_ID`);

ALTER TABLE `Biospecimen` ADD FOREIGN KEY (`Biospecimen_ID`) REFERENCES `Collection_Tube` (`Biospecimen_ID`);

ALTER TABLE `Aliquot` ADD FOREIGN KEY (`Aliquot_ID`) REFERENCES `Aliquot_Tube` (`Aliquot_ID`);
